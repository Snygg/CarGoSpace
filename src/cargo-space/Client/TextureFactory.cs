...
namespace CargoSpace.Client
{
    public class TextureFactory
    {
        ...
        private TileSet CreateAtmosphereTileSet()
        {
            ...
            AddAtmosphereSource(tileSet, 0, "ZeroOxygen", new Color(0.0f, 0.0f, 0.0f, 0.55f));
            AddAtmosphereSource(tileSet, 1, "LowOxygen", new Color(0.65f, 0.85f, 1.0f, 0.55f));
            AddAtmosphereSource(tileSet, 2, "Smoke", new Color(0.7f, 0.7f, 0.7f, 0.45f));

            return tileSet;
        }

        private void AddAtmosphereSource(TileSet tileSet, int id, string name, Color color)
        {
            Image image = Image.CreateEmpty(Constants.TileSize, Constants.TileSize, false, Image.Format.Rgba8);
            ...
            // Use modulate instead of a per-tile shader; this is reliable across Godot versions
            // and still gives a colored transparent overlay over the tile.
            atlasSource.GetTileData(new Vector2I(0, 0), 0).Modulate = color;
